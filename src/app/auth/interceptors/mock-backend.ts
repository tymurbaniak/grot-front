...
const users = JSON.parse(localStorage.getItem(usersKey) as string) || [];
...
export class MockAuthenticationInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                ...
                case url.endsWith('/auth/refresh-token') && method === 'POST':
                    return refreshToken();
                case url.endsWith('/auth/revoke-token') && method === 'POST':
                    return revokeToken();
        ...
        function authenticate() {
            ...
            if (!user) return MockResponses.error('Username or password is incorrect');

            // add refresh token to user
            user.refreshTokens.push(generateRefreshToken());
            localStorage.setItem(usersKey, JSON.stringify(users));

            return MockResponses.ok({
                id: user.id,
        ...
        function refreshToken() {
            ...
            if (!refreshToken) return MockResponses.unauthorized();

            const user = users.find((x: any) => x.refreshTokens.includes(refreshToken));

            if (!user) return MockResponses.unauthorized();
        ...
        function revokeToken() {
            if (!isLoggedIn()) return MockResponses.unauthorized();
